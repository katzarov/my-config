...
if [ -z "$VENDOR_ID" ]; then
    echo "Could not determine Keychron vendor ID."
    exit 1
fi
...
# # Create minimal rule for Ubuntu 24 instead - does not work dont care acutlaly
# cat <<EOF > "$RULE_FILE"
# # Keychron Keyboard/Mouse udev rule for VIA / Keychron Launcher
# KERNEL=="hidraw*", SUBSYSTEM=="hidraw", ATTRS{idVendor}=="$VENDOR_ID", TAG+="uaccess"
# EOF
